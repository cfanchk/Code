...
void calFeature(float* matrix, float* featureVector, const float* meanVector, int product)
{
...
int main()
{
	char* videoDst = "D:\\lawn_train_1.avi";
	...
	if (overwrite)
		outfile.open(feaName, ios::out);
	else
		outfile.open(feaName, ios::app);
	if (!outfile)
	{
	...
	while (1)
	{
		...
		if (!preframe.empty())
		{
			...
			*(meanVector + 1) = ((float)cols + 1) / 2;
			...
			calFeature(matrix, featureVector, meanVector, product);
			write2file(featureVector);
		}
		frame.copyTo(preframe);
		imshow("Train", colorframe);
		waitKey(1);
		framenum++;
		cout << framenum << endl;
